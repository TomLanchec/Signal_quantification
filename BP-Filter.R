...
read_any <- function(f) {
  ...
  df %>%
    mutate(
      PolII_Ser2 = Mean_Green / Mean_DAPI,     # vert
      V5_TBP1    = Mean_Red   / Mean_DAPI,     # rouge
      SourceFile = tools::file_path_sans_ext(basename(f))
    ) %>%
    filter(is.finite(PolII_Ser2), is.finite(V5_TBP1))
}
...
dat_long <- all_dat %>%
  select(SourceFile, Condition, ROI_Index, PolII_Ser2, V5_TBP1) %>%
  pivot_longer(c(PolII_Ser2, V5_TBP1), names_to = "Channel", values_to = "Norm")

#Filtre
dat_long <- dat_long %>% filter(between(Norm, 0, 40))
...
dat_long <- dat_long %>%
  ...
  mutate(
    ...
    Channel = factor(Channel, levels = c("PolII_Ser2","V5_TBP1"),
                     labels = c("PolII_Ser2","V5-TBP1"))
  )
...
# ---------- Plot 1 : PolII_Ser2 ----------
df_pol <- dat_long %>% filter(Channel == "PolII_Ser2")
p_pol <- ggplot(df_pol, aes(x = Time, y = Norm)) +
  ...
  labs(title = "PolII_Ser2 (0–5)",
       x = "time (min)", y = "Mean(PolII_Ser2) / Mean(DAPI)")
print(p_pol)

ggsave(file.path(input_dir, "BOX_PolII_Ser2_by_time_0-5.png"),
