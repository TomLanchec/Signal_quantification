// ===== SIMPLE Fiji Macro v5: Measure per channel by switching channels (no redirect/duplicates) =====
// - Segmentation on a DAPI mask (from the same image; Z-proj optional)
// - Measurement done directly on the source hyperstack by switching channels with Stack.setChannel()
// - Saves CSV to a fixed folder in your home (edit SAVE_DIR if needed)
...
// -------- Dialog --------
Dialog.create("Simple per-nucleus quant (Green/Red normalized by DAPI) - v5 setChannel");
...
// -------- Open / prepare source --------
if (nImages==0) { open(); if (nImages==0) exit("No image open."); }
srcTitle = getTitle();
Stack.getDimensions(w,h,C,Z,T);
if (dapiIdx<1 || dapiIdx>C || gIdx<1 || gIdx>C || rIdx<1 || rIdx>C) exit("Channel index out of range.");
...
if (Z>1 && doZProj) {
    ...
    close(srcTitle);
    rename(srcTitle);
    Stack.getDimensions(w,h,C,Z,T); // now Z should be 1
}

// Optional preprocessing on the source image (affects measurements)
selectWindow(srcTitle);
if (doBG) run("Subtract Background...", "rolling="+rbRad+" stack");
if (doMed) run("Median...", "radius="+medRad+" stack");

// -------- Build DAPI mask for segmentation --------
...
if (nROIs==0) exit("No nuclei found. Adjust threshold/min area");
close("DAPI_mask");

// -------- Measurements on the source image by switching channels --------
selectWindow(srcTitle);
...
nD = nResults;
A  = newArray(nD); MD = newArray(nD); ID = newArray(nD);
